...
本项目处于 Demo 架构和数据准备阶段，项目名称尚未最终确定。当前目标是围绕主路径完成可运行后端 Demo：

```text
1 款茶（铁观音）× 图片物料 ×（国内链 + 跨文化链）两条同构链路
```

两条链路均完整跑通：国内链面向国内消费者，跨文化链面向欧美精品咖啡爱好者。两条链共享同一款茶的知识与风味坐标，跨文化表达由国内表达按规则横向翻译派生而来。
...
3. 写 seed 文件和 `seed.py --reset`，包括知识数据、规则数据（含“国内表达→国外表达”翻译规则）和 mock 输出（国内链与跨文化链各一套）。
4. 实现规则筛选函数，例如按 `scope/market/audience_reference/trigger_terms` 取相关规则。
5. 用 mock 数据实现 P0 API（国内表达、跨文化表达、中/英文物料、追溯）。
6. 实现 `/api/*` fallback。
7. 增加纵向追溯链（国内链与跨文化链各四层）；跨文化表达的 `source_expression_id` 为横向翻译引用，不写入纵向 trace_links。
8. 后续再接 LLM 和内存缓存。
