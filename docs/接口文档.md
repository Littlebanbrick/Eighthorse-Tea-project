# Demo 接口文档

本文档用于前后端协作，描述 Demo 阶段后端 API 的功能边界、请求参数、响应结构和 fallback 规则。

Demo 当前主路径为：

```text
1 款茶（铁观音） × 欧美市场 × 咖啡爱好者 × 图片物料
```

后端会优先保证这条路径完整跑通。其他茶品、市场、受众参照系、视频生成等能力可以先由前端做好入口，后端通过 fallback 返回“暂未开放”的标准响应，避免页面空白或报错。

## 1. 通用约定

### 1.1 Base URL

本地开发默认：

```text
http://localhost:8000
```

所有业务接口统一以 `/api` 开头。

### 1.2 数据格式

请求和响应均使用 JSON：

```http
Content-Type: application/json
```

### 1.3 ID 约定

Demo 阶段建议使用可读字符串 ID，而不是纯数字 ID，方便前后端调试和追溯。

示例：

```text
tieguanyin_001
flavor_tieguanyin_001
expr_cn_tieguanyin_001
expr_en_tieguanyin_coffee_001
asset_tieguanyin_poster_001
```

### 1.4 通用响应字段

正常响应建议包含：

```json
{
  "success": true,
  "data": {},
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

fallback 响应建议包含：

```json
{
  "success": true,
  "data": {},
  "meta": {
    "demo_mode": true,
    "fallback": true,
    "fallback_reason": "该功能 Demo 阶段暂未开放"
  }
}
```

错误响应建议包含：

```json
{
  "success": false,
  "error": {
    "code": "TEA_NOT_FOUND",
    "message": "未找到对应茶品"
  }
}
```

## 2. Demo 路径接口

### 2.1 获取当前支持的 Demo 路径

```http
GET /api/demo-routes
```

用途：

前端用于判断当前 Demo 支持哪些完整链路。当前阶段只返回铁观音、欧美市场、咖啡爱好者、图片物料这一条路径。

响应示例：

```json
{
  "success": true,
  "data": [
    {
      "id": "tieguanyin_western_coffee_poster",
      "tea_id": "tieguanyin_001",
      "tea_name": "赛珍珠铁观音",
      "market": "western",
      "target_language": "en",
      "audience_reference": "specialty_coffee_lovers",
      "asset_type": "poster",
      "enabled": true,
      "description": "铁观音面向欧美精品咖啡爱好者的英文图片物料 Demo 路径"
    }
  ],
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

前端建议：

如果某个页面入口不在 `enabled=true` 的路径中，可以仍然展示入口，但点击后调用 fallback 页面或 fallback API。

## 3. 茶品与知识接口

### 3.1 获取茶品列表

```http
GET /api/teas
```

用途：

返回 Demo 数据库中已有茶品。前端首页、茶品选择器使用该接口。

响应示例：

```json
{
  "success": true,
  "data": [
    {
      "id": "tieguanyin_001",
      "name": "赛珍珠铁观音",
      "category": "乌龙茶",
      "origin": "福建安溪",
      "brand": "八马茶业",
      "demo_available": true
    }
  ],
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

字段说明：

- `id`：茶品唯一 ID。
- `demo_available`：该茶是否有完整 Demo 链路。

### 3.2 获取茶品知识卡片

```http
GET /api/teas/{tea_id}/knowledge
```

用途：

返回某款茶的基础知识卡片，包括基础信息、产地、工艺、文化故事和数据来源。对应四层架构中的“知识 / 证据层”。

路径参数：

- `tea_id`：茶品 ID，例如 `tieguanyin_001`。

响应示例：

```json
{
  "success": true,
  "data": {
    "tea": {
      "id": "tieguanyin_001",
      "name": "赛珍珠铁观音",
      "category": "乌龙茶",
      "origin": "福建安溪",
      "brand": "八马茶业"
    },
    "origin": {
      "region": "福建安溪",
      "terroir": "山地丘陵、云雾充足、昼夜温差较明显，适宜乌龙茶香气形成"
    },
    "process": {
      "name": "铁观音传统制作工艺",
      "steps": ["晒青", "凉青", "摇青", "杀青", "揉捻", "焙火"],
      "key_technique": "看青做青、摇青与焙火控制"
    },
    "story": {
      "title": "铁观音非遗制茶技艺",
      "content": "铁观音制作技艺强调经验判断与工艺控制，是中国乌龙茶的重要代表。"
    },
    "evidence": [
      {
        "id": "evidence_tea_flavor_wheel_2022",
        "source_type": "public_standard",
        "title": "T/CTSS 58-2022 茶叶感官风味轮",
        "source": "T/CTSS 58-2022 茶叶感官风味轮",
        "confidence": "high",
        "note": "Demo 阶段用于风味属性和审评术语参考。source_type、source、confidence 由团队人工确认。"
      }
    ]
  },
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

## 4. 风味结构化接口

### 4.1 获取风味坐标卡

```http
GET /api/teas/{tea_id}/flavor-profile
```

用途：

返回某款茶的结构化风味坐标，用于前端绘制雷达图、风味卡片，也作为后续中文表达、跨文化表达和营销物料生成的依据。

响应示例：

```json
{
  "success": true,
  "data": {
    "profile_id": "flavor_tieguanyin_001",
    "tea_id": "tieguanyin_001",
    "dimensions": [
      {
        "key": "floral",
        "label_zh": "花香",
        "label_en": "Floral",
        "intensity": 8,
        "description": "以兰花香为代表的清雅花香",
        "evidence_ids": ["evidence_tea_flavor_wheel_2022"]
      },
      {
        "key": "freshness",
        "label_zh": "鲜爽度",
        "label_en": "Freshness",
        "intensity": 7,
        "description": "入口清爽，回味较明亮",
        "evidence_ids": ["evidence_public_oolong_components"]
      },
      {
        "key": "finish",
        "label_zh": "回甘与尾韵",
        "label_en": "Lingering Finish",
        "intensity": 8,
        "description": "饮后口腔留香，尾韵较长",
        "evidence_ids": ["evidence_tea_flavor_wheel_2022"]
      }
    ],
    "component_notes": [
      {
        "name": "氨基酸",
        "value_type": "typical_range",
        "description": "Demo 阶段采用公开文献中的乌龙茶典型范围，不代表八马单品实测值"
      }
    ]
  },
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

前端建议：

- `dimensions[].intensity` 取值范围建议为 `0-10`。
- 雷达图可优先使用 `label_zh` 或 `label_en`。
- 如果需要展示依据，可以根据 `evidence_ids` 调用追溯接口。

## 5. 表达生成接口

说明：

表达生成接口内部会使用结构化规则库。前端不需要直接传入规则，也不需要拼接 prompt。后端会根据当前任务、茶品、市场、受众参照系和茶品术语，从 `generation_rules` 中筛选相关规则，再把少量匹配规则注入 LLM prompt。

例如：

```text
铁观音 × 欧美市场 × 精品咖啡爱好者 × 跨文化表达
```

后端会优先匹配：

```text
scope = cross_cultural_expression
market = western 或 any
audience_reference = specialty_coffee_lovers 或 any
trigger_terms 命中 观音韵 / 花香 / 回甘 等术语
```

这部分属于后端内部工作流，不作为前端必传字段。

### 5.1 生成国内中文表达

```http
POST /api/teas/{tea_id}/domestic-expression
```

用途：

根据茶品和客户画像，生成面向国内消费者的中文表达。对应“感知 → 具象化语言”。

请求体示例：

```json
{
  "audience": {
    "age_group": "gen_z",
    "knowledge_level": "beginner",
    "scenario": "self_drinking",
    "psychology": "curiosity"
  },
  "style": "store_sales"
}
```

响应示例：

```json
{
  "success": true,
  "data": {
    "expression_id": "expr_cn_tieguanyin_001",
    "tea_id": "tieguanyin_001",
    "audience": {
      "age_group": "gen_z",
      "knowledge_level": "beginner",
      "scenario": "self_drinking",
      "psychology": "curiosity"
    },
    "outputs": {
      "story_style": "这款铁观音可以先从香气理解：它不是浓烈的香水感，而是入口后慢慢展开的兰花香。",
      "scientific_style": "它的鲜爽、回甘和花香来自品种、做青和焙火共同作用。Demo 阶段的成分说明采用公开文献代理数据。",
      "emotional_style": "适合在下午慢慢喝，第一口是清爽，后面留下的是比较柔和的花香和回甘。"
    },
    "source_profile_id": "flavor_tieguanyin_001",
    "trace_id": "expr_cn_tieguanyin_001"
  },
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

### 5.2 生成跨文化表达

```http
POST /api/teas/{tea_id}/cross-cultural-expression
```

用途：

根据茶品、目标语言和受众参照系，生成面向海外受众的表达。Demo 阶段优先支持英语和欧美精品咖啡爱好者。

请求体示例：

```json
{
  "target_language": "en",
  "market": "western",
  "audience_reference": "specialty_coffee_lovers",
  "audience_level": "beginner",
  "preserve_chinese_terms": true
}
```

响应示例：

```json
{
  "success": true,
  "data": {
    "translation_id": "expr_en_tieguanyin_coffee_001",
    "tea_id": "tieguanyin_001",
    "target_language": "en",
    "market": "western",
    "audience_reference": "specialty_coffee_lovers",
    "outputs": {
      "literal_explanation": "Guanyin Yun refers to the lingering impression Tieguanyin leaves after drinking, including aroma, aftertaste, and mouthfeel.",
      "beginner_analogy": "For specialty coffee drinkers, think of it as a clean floral finish: not heavy, but memorable after the sip.",
      "cultural_narrative": "In Chinese tea culture, Yun is not only flavor. It describes the rhythm and aftertaste that stay with the drinker."
    },
    "analogy_rules": [
      {
        "source_dimension": "花香",
        "target_reference": "floral finish in washed coffee",
        "confidence": "medium",
        "note": "用于降低理解门槛，不等同于完全相同的风味物质"
      }
    ],
    "source_profile_id": "flavor_tieguanyin_001",
    "trace_id": "expr_en_tieguanyin_coffee_001"
  },
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

fallback 场景：

- `target_language` 不是 `en`。
- `market` 不是 `western`。
- `audience_reference` 不是 `specialty_coffee_lovers`。

这些场景下后端可以返回标准 fallback，而不是报错。

## 6. 营销物料接口

### 6.1 生成图片物料

```http
POST /api/teas/{tea_id}/marketing-asset
```

用途：

生成营销图片物料所需的数据。Demo 阶段不强依赖真实生图 API，后端可以返回海报文案、雷达图数据、图片生成 prompt，由前端模板渲染。

请求体示例：

```json
{
  "route_id": "tieguanyin_western_coffee_poster",
  "asset_type": "poster",
  "platform": "tiktok",
  "language": "en",
  "style": "premium_but_approachable"
}
```

响应示例：

```json
{
  "success": true,
  "data": {
    "asset_id": "asset_tieguanyin_poster_001",
    "tea_id": "tieguanyin_001",
    "asset_type": "poster",
    "platform": "tiktok",
    "language": "en",
    "copy": {
      "headline": "Tieguanyin, Explained Like Specialty Coffee",
      "subheadline": "Floral aroma, clean freshness, and a lingering finish.",
      "body": "A Chinese oolong tea with a graceful floral profile and a finish that stays after the sip."
    },
    "visual_data": {
      "radar": [
        { "label": "Floral", "value": 8 },
        { "label": "Freshness", "value": 7 },
        { "label": "Lingering Finish", "value": 8 },
        { "label": "Roasted Note", "value": 4 }
      ]
    },
    "image_prompt": "Premium poster for Chinese Tieguanyin oolong tea, designed for Western specialty coffee lovers, elegant tea leaves, porcelain cup, clear floral mood, modern editorial layout.",
    "source_translation_id": "expr_en_tieguanyin_coffee_001",
    "trace_id": "asset_tieguanyin_poster_001"
  },
  "meta": {
    "demo_mode": true,
    "fallback": false,
    "image_generation_enabled": false
  }
}
```

字段说明：

- `copy`：前端直接展示的海报文案。
- `visual_data.radar`：前端绘制雷达图用。
- `image_prompt`：后续接生图 API 时可直接使用。
- `image_generation_enabled=false`：表示当前未真正调用生图服务。

## 7. 追溯接口

### 7.1 查询输出依据链

```http
GET /api/trace/{output_id}
```

用途：

查询某个输出的来源链路。用于体现“AI 只负责转译，事实来自结构化知识库”。

路径参数：

- `output_id`：任意可追溯输出 ID，例如 `expr_en_tieguanyin_coffee_001` 或 `asset_tieguanyin_poster_001`。

响应示例：

```json
{
  "success": true,
  "data": {
    "output_id": "asset_tieguanyin_poster_001",
    "output_type": "marketing_asset",
    "trace": [
      {
        "level": 4,
        "name": "多模态物料",
        "id": "asset_tieguanyin_poster_001",
        "summary": "面向欧美咖啡爱好者的英文图片物料"
      },
      {
        "level": 3,
        "name": "跨文化表达",
        "id": "expr_en_tieguanyin_coffee_001",
        "summary": "用 specialty coffee 的 floral finish 降低理解门槛，同时保留 Guanyin Yun 的文化解释"
      },
      {
        "level": 2,
        "name": "中文具象化表达",
        "id": "expr_cn_tieguanyin_001",
        "summary": "兰花香、清爽、尾韵较长"
      },
      {
        "level": 1,
        "name": "风味坐标",
        "id": "flavor_tieguanyin_001",
        "summary": "花香 8，鲜爽度 7，回甘与尾韵 8"
      },
      {
        "level": 0,
        "name": "知识依据",
        "id": "knowledge_tieguanyin_001",
        "summary": "铁观音工艺、乌龙茶审评术语、公开文献代理数据"
      }
    ]
  },
  "meta": {
    "demo_mode": true,
    "fallback": false
  }
}
```

前端建议：

可以在表达结果或海报旁边放一个“查看依据”按钮，调用该接口展示追溯链。

## 8. Fallback 接口与规则

### 8.1 通用 fallback

```http
GET /api/fallback
POST /api/fallback
```

用途：

当前端访问尚未开放的功能时，返回统一占位内容。适用于视频生成、多语言扩展、非 Demo 茶品、非咖啡受众参照系等。

请求体示例：

```json
{
  "feature": "video_generation",
  "requested_path": "/api/teas/tieguanyin_001/video-asset",
  "reason": "frontend_placeholder"
}
```

响应示例：

```json
{
  "success": true,
  "data": {
    "title": "功能暂未开放",
    "message": "该能力已在产品规划中，Demo 阶段暂不提供真实生成结果。",
    "suggested_action": "请返回当前 Demo 主路径：铁观音 × 欧美市场 × 咖啡爱好者 × 图片物料。",
    "available_route_id": "tieguanyin_western_coffee_poster"
  },
  "meta": {
    "demo_mode": true,
    "fallback": true,
    "fallback_reason": "feature_not_available"
  }
}
```

### 8.2 未开放功能的推荐路由

以下接口可以先不实现真实逻辑，但建议后端注册路由并返回 fallback，避免前端点击时报 404。

```http
POST /api/teas/{tea_id}/video-asset
POST /api/translate
POST /api/image/generate
POST /api/audio/generate
GET  /api/markets
GET  /api/audience-references
```

推荐响应：

```json
{
  "success": true,
  "data": {
    "title": "功能暂未开放",
    "message": "当前 Demo 暂不支持该功能，后续可由后端接入真实能力。",
    "available_route_id": "tieguanyin_western_coffee_poster"
  },
  "meta": {
    "demo_mode": true,
    "fallback": true,
    "fallback_reason": "feature_not_available"
  }
}
```

### 8.3 404 自动 fallback

后端可以设置全局 404 处理器。当前端访问不存在的 `/api/*` 路由时，不直接返回默认 404，而是返回 fallback JSON。

建议响应：

```json
{
  "success": true,
  "data": {
    "title": "接口暂未开放",
    "message": "该接口尚未在 Demo 后端中实现。请确认是否属于后续功能。",
    "available_route_id": "tieguanyin_western_coffee_poster"
  },
  "meta": {
    "demo_mode": true,
    "fallback": true,
    "fallback_reason": "api_not_implemented"
  }
}
```

注意：

只建议对 `/api/*` 做 fallback。非 API 路由仍可按前端框架自己的路由规则处理。

## 9. 前后端协作优先级

### P0：必须完成

```http
GET  /api/demo-routes
GET  /api/teas
GET  /api/teas/{tea_id}/knowledge
GET  /api/teas/{tea_id}/flavor-profile
POST /api/teas/{tea_id}/cross-cultural-expression
POST /api/teas/{tea_id}/marketing-asset
GET  /api/trace/{output_id}
```

### P1：建议完成

```http
POST /api/teas/{tea_id}/domestic-expression
GET  /api/fallback
POST /api/fallback
```

### P2：占位即可

```http
POST /api/teas/{tea_id}/video-asset
POST /api/translate
POST /api/image/generate
POST /api/audio/generate
GET  /api/markets
GET  /api/audience-references
```

## 10. 实现原则

1. SQLite 数据库只作为运行产物，不作为人工维护的数据源。
2. Demo 数据应来自可提交到 Git 的 YAML 或 JSON seed 文件。
3. 关键资料应带 `source_type`、`source`、`confidence` 等轻量证据字段；Demo 阶段这些字段由团队人工确认，Agent 只辅助整理。
4. 生成规则应作为结构化数据维护在 seed 文件和 SQLite 中，例如 `generation_rules`；不要长期硬编码在 Python 或超长 prompt 中。
5. 后端生成表达时，应按任务、市场、受众和触发术语筛选少量相关规则，再注入 prompt。
6. 所有 LLM 生成结果必须尽量包含 `source_profile_id` 或 `trace_id`。
7. Demo 阶段不要让未开放功能报错或白屏，统一返回 fallback。
8. 前端不要依赖自然语言解析，所有关键内容都应放在稳定 JSON 字段中。
9. 后端可以先返回静态 mock 数据，再逐步替换为数据库查询、规则筛选和 LLM 调用。
